...
class PurchaseRemoteDatasource {
  const PurchaseRemoteDatasource(this._apiClient);

  final ApiClient _apiClient;

  Future<PurchaseModel> createPurchase({required RestockRequestModel request}) async {
    try {
      final response = await _apiClient.post<Map<String, dynamic>>(
        ApiConstants.purchases,
        data: request.toJson(),
      );
      return PurchaseModel.fromJson(_extractMap(response.data));
